...
<title>{title} · QoE 노멀라이저</title>
<style>
  :root{{
    --ink:#1a1f2b; --ink-soft:#4a5262; --line:#e3e7ee; --line-2:#eef1f6;
    --bg:#f6f7f9; --card:#ffffff; --accent:#1f5f8b; --accent-soft:#e8f0f6;
    --good:#217a4b; --warn:#9a6a12; --bad:#a5342b; --mute:#8b93a3;
  }}
  *{{box-sizing:border-box}}
  ...
  .brand span{{font-size:12.5px;color:var(--mute)}}
  h1{{font-size:22px;letter-spacing:-.01em;margin:26px 0 6px;text-wrap:balance}}
  ...
  button,.btn{{display:inline-block;border:0;border-radius:9px;background:var(--accent);
    color:#fff;font-size:14.5px;font-weight:600;padding:11px 20px;cursor:pointer;
    font-family:inherit;text-decoration:none;margin-top:20px}}
  button:hover,.btn:hover{{background:#194e73}}
  .btn.ghost{{background:transparent;color:var(--accent);padding:6px 0;margin-top:0;
    font-size:13px}}
  a{{color:var(--accent)}}
  ...
  .keystate{{display:flex;align-items:center;gap:8px;font-size:13px;color:var(--ink-soft);
    background:var(--line-2);border-radius:8px;padding:9px 13px;margin-bottom:4px}}
  .dot{{width:8px;height:8px;border-radius:50%;background:var(--good);flex:none}}
  .err{{background:#fbeeec;border:1px solid #f0cfca;color:var(--bad);border-radius:9px;
  ...
  li.running .mark{{background:var(--accent);color:#fff;
    animation:pulse 1.3s ease-in-out infinite}}
  li.done .mark{{background:var(--good);color:#fff}}
  ...
  <div class="brand"><b>QoE 노멀라이저</b><span>영업이익 질 스크리닝 · 로컬</span></div>
  {body}
...
def preview():
    ...
    if fr.get("has_saved") and fresh is True:
        head = ('<div class="keystate" style="background:#e8f4ec;color:#1a5c37">'
        ...
        head = ('<div class="keystate" style="background:#fbf3e6;color:#7a5410">'
  ...
  <div class="card">
    ...
    <a class="btn ghost" href="/" style="margin-top:8px">← 다른 회사</a>
  </div>"""
